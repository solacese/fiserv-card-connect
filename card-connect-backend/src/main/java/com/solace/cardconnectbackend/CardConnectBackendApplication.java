...
public class CardConnectBackendApplication {
	 ...
	 * 
	 * String topic =
	 * "terminal/".concat(deviceid).concat("/tranaction/").concat(type);
	...
	public Function<Message<String>, Message<String>> initializeTransaction() {
		return v -> {
			...
			String topic = "cardpointe/terminal/".concat(deviceid).concat("/transaction/").concat(transType);
			...
			builder = (MessageBuilder<String>) MessageBuilder.withPayload(respObj.toString())
	...
	public Function<Message<String>, Message<String>> processTransaction() {
		return v -> {
			...
			MessageBuilder<String> builder;
			builder = (MessageBuilder<String>) MessageBuilder.withPayload(respObj.toString())
					.setHeader(BinderHeaders.TARGET_DESTINATION, destinationTopic.toString())
					.setHeader(REPLYTO_DESTINATION_KEY, replyToTopic);
			return builder.build();
		};
	}
